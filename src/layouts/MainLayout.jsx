...
const I = {
  ...
  shirt: 'M15.75 4.5l-7.5 3.75L12 12.75l3.75-4.5-3.75-3.75zM7.5 8.25L4.5 12l3 3.75L12 12l-4.5-3.75zM16.5 8.25L19.5 12l-3 3.75L12 12l4.5-3.75zM12 12.75l-3.75 4.5 7.5 3.75 3.75-7.5-7.5-.75z',
}
...
const navItems = {
  ...
  headmaster: [
    ...
    { path: '/headmaster/teachers', icon: I.teachers, label: 'Teachers' },
    { path: '/headmaster/students', icon: I.students, label: 'Students' },
  ...
  useEffect(() => {
    supabase.from('school_settings').select('logo_url, school_name').limit(1).then(({ data }) => {
      if (data?.[0]) {
        ...
        if (data[0].school_name) document.title = data[0].school_name
      }
    })
  }, [])
    ...
    <div className="h-screen bg-gray-50 flex overflow-hidden">
      ...
      `}>
        ...
        <div className="flex items-center gap-3 px-5 h-16 border-b border-gray-100 shrink-0">
          {schoolInfo?.logo_url ? (
            <img src={schoolInfo.logo_url} alt="" className="w-9 h-9 object-contain shrink-0" crossOrigin="anonymous" />
          ) : (
          ...
          <div>
            <p className="text-sm font-bold text-gray-900 leading-tight">{schoolInfo?.school_name || 'Kizaga RMS'}</p>
      ...
      <div className="flex-1 flex flex-col overflow-hidden lg:ml-0">
        {/* Top bar */}
        <header className="no-print flex items-center justify-between h-14 sm:h-16 px-4 bg-white border-b border-gray-200 shrink-0">
