...
function attachAssignedStudentsToSubjects(subjects, students, subjectAssignments) {
  const studentsById = new Map((students || []).map(student => [student.id, student]))
  return (subjects || []).map(subject => {
    const isAssignedSubject = subject.subject_type !== 'COMPULSORY' && subject.subject_role !== 'CORE'
    if (!isAssignedSubject) return subject

    const assignedIds = subjectAssignments?.get(subject.id) || []
    if (assignedIds.length === 0) return subject
    return {
      ...subject,
      students: sortStudentsByGenderAndName(assignedIds.map(id => studentsById.get(id)).filter(Boolean)),
    }
  })
}

async function fetchActiveStudents(filters = {}) {
  const PAGE = 1000
  let from = 0
  const all = []
...
async function fetchStudentSubjectAssignments(studentIds, subjectIds) {
  const PAGE = 1000
  const CHUNK = 50
  const all = []

  if (!studentIds?.length || !subjectIds?.length) return all
    ...
    try {
      ...
      if (isALevel) {
        ...
        for (const cs of classStreamsList) {
          ...
          const stuData = await fetchActiveStudents({ class_stream_id: cs.id })
          const students = sortStudentsByGenderAndName(stuData)
          const studentIds = students.map(s => s.id)
          const subjectIds = subjects.map(s => s.id)

          const subjectAssignmentRows = await fetchStudentSubjectAssignments(studentIds, subjectIds)
          const subjectAssignments = new Map()
          ;(subjectAssignmentRows || []).forEach(row => {
            if (!subjectAssignments.has(row.subject_id)) subjectAssignments.set(row.subject_id, [])
            subjectAssignments.get(row.subject_id).push(row.student_id)
          })
          const subjectsWithStudents = attachAssignedStudentsToSubjects(subjects, students, subjectAssignments)
        ...
        const stuData = await fetchActiveStudents({ class_id: classId })
        const students = sortStudentsByGenderAndName(stuData)
        ...
        openAttendancePrintWindow({ exam, selectedClass, students, subjects: subjectsWithStudents, schoolInfo, logoBase64, isALevel: false })
      }
    } catch (err) {
  ...
  if (isALevel && streamGroups?.length > 0) {
    for (const { streamName, subjects: streamSubjects, students: streamStudents } of streamGroups) {
      for (const subject of streamSubjects) {
        sheets += buildSheet({ logoBase64, schoolName, addressLine, exam, className, streamLabel: streamName, subject, students: subject.students || streamStudents })
      }
    }
  } else {
    for (const subject of (subjects || [])) {
      sheets += buildSheet({ logoBase64, schoolName, addressLine, exam, className, streamLabel: null, subject, students: subject.students || students || [] })
    }
  }
