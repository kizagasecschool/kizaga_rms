  ...
  useEffect(() => {
    ...
    const loadStudentsAndMarks = async () => {
      ...
      try {
        ...
        const selectedSubjectMeta = subjectOptions.find(s => s.id === selectedSubjectId)
        const isOptionalOrElective = selectedSubjectMeta?.subject_type
          ? selectedSubjectMeta.subject_type !== 'COMPULSORY'
          : false

        let studsQuery = supabase.from('students').select('*').eq('status', 'active')

        if (isClassWide && isOLevel && !isOptionalOrElective) {
          // O-Level compulsory: all students in the class take the subject — filter by class_id only
          studsQuery = studsQuery.eq('class_id', classId)
        } else {
          // Determine the pool of student IDs to check for subject assignment
          let poolIds
          if (isClassWide && isOLevel) {
            // Optional/elective O-Level subject — use the whole class as the pool
            const { data: classStudents } = await supabase
              .from('students')
              .select('id')
              .eq('class_id', classId)
              .eq('status', 'active')
            poolIds = (classStudents || []).map(r => r.id)
          } else {
            // A-Level / individual stream: pool is the students in these streams
            poolIds = streamIds.length > 0
              ? ((await supabase.from('students').select('id').in('class_stream_id', streamIds).eq('status', 'active')).data || []).map(r => r.id)
              : []
          }

          const assignedIds = await fetchAssignedStudentIds(selectedSubjectId, poolIds)
          ...
          studsQuery = supabase
            ...
            .select('*')
            .eq('status', 'active')
            .in('id', assignedIds.length > 0 ? assignedIds : [])
        }
