      ...
      try {
        ...
        const fetchStudents = async () => {
          ...
          if (oLevelClassIds.length > 0)
            await fetchChunk(supabase.from('students').select('*').in('class_id', oLevelClassIds).eq('status', 'active'))
          if (aLevelStreamIds.length > 0)
            await fetchChunk(supabase.from('students').select('*').in('class_stream_id', aLevelStreamIds).eq('status', 'active'))

          return rows
        }

        const [studentSubs, subRes, students] = await Promise.all([
