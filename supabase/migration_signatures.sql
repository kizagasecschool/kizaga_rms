...
-- Storage policies for signatures bucket (idempotent - safe to re-run)
DROP POLICY IF EXISTS "public_select_signatures" ON storage.objects;
CREATE POLICY "public_select_signatures" ON storage.objects
  FOR SELECT USING (bucket_id = 'signatures');

DROP POLICY IF EXISTS "admin_insert_signatures" ON storage.objects;
CREATE POLICY "admin_insert_signatures" ON storage.objects
  FOR INSERT WITH CHECK (
    bucket_id = 'signatures'
    AND (SELECT get_my_role()) IN ('admin', 'headmaster', 'academic')
  );

DROP POLICY IF EXISTS "admin_update_signatures" ON storage.objects;
...
DROP POLICY IF EXISTS "admin_delete_signatures" ON storage.objects;
